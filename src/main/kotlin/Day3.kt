...
private infix fun Int.approaching(other: Int): List<Int> {
...
private data class Segment(val a: Point, val b: Point) {
    val intermediates
        get() = (a.x approaching b.x) * (a.y approaching b.y)

    fun getIntersections(other: Segment) =
        intermediates.intersect(other.intermediates)
}
...
private data class Wire(val points: List<Point>) {
    val segments
        get() = points.dropLast(1)
            .zip(points.drop(1))
            .map { (a, b) -> Segment(a, b) }

    fun getIntersections(other: Wire) =
        (segments * other.segments)
            .asSequence()
            .map { (a, b) -> a.getIntersections(b) }
    ...
    fun getLowestManhattanDistance(other: Wire) = getIntersections(other)
        .map { it.manhattanDistance }
        .min()
}

private class InstructionList(val instructions: List<Instruction>) {
    val wire: Wire
        get() {
            val points = instructions.fold(listOf(Point(1, 1))) { wire, (direction, distance) ->
                val nextPoint = wire.last() + when (direction) {
                    Direction.Right -> Point(distance, 0)
                    Direction.Left -> Point(-distance, 0)
    ...
    companion object {
        fun fromString(instructionString: String): InstructionList {
...
fun showLowestIntersectionDistance() {
    val wire1 = InstructionList.fromString(wireInput.first).wire
    val wire2 = InstructionList.fromString(wireInput.second).wire
    val dist = wire1.getLowestManhattanDistance(wire2)
    println("lowest intersection distance: $dist")
}

fun main() {
    showLowestIntersectionDistance()
}