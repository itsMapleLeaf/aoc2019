...
private fun getIntersections(first: Wire, second: Wire) =
    first.points.intersect(second.points) - Point.origin // origin always intersects, take it out

private fun getLowestManhattanDistance(points: Iterable<Point>) =
    points.map { it.manhattanDistance }.min()

private val wireInput = Pair(
...
private fun showLowestIntersectionDistance() {
    val wire1 = Wire(wireInput.first)
    val wire2 = Wire(wireInput.second)
    println(getLowestManhattanDistance(getIntersections(wire1, wire2)))
}
