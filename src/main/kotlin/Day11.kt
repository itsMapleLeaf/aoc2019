...
private fun movementVector(angle: Double): Vector {
    val x = sin(angle)
    val y = cos(angle)
    return Vector(x.toInt(), -y.toInt())
}

private fun getPaintedSquares(startingColor: Int): Map<Vector, Int> {
    var robotPosition = Vector(0, 0)
    var direction = 0.0
    var program = IntcodeProgram.fromString(robotProgram)
    val paintedSquares = mutableMapOf(Vector(0, 0) to startingColor)
...
private fun getImage(squares: Map<Vector, Int>): String {
